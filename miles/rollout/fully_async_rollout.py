...
class _CachedWeightVersion:
    ...
    def __init__(self, ttl: float = 1.0):
        ...
        self._last_query_succeeded: bool | None = None
        self._lock = asyncio.Lock()

    @property
    def available(self) -> bool:
        """Whether the latest router query produced an authoritative version."""
        return self._value is not None and self._last_query_succeeded is True

    @property
    def success_age_seconds(self) -> float | None:
        if self._last_success == float("-inf"):
            return None
...
class FullyAsyncRolloutFn:
    ...
    async def _drain(self, rollout_id: int) -> RolloutFnTrainOutput:
        ...
        current_version_available: list[int] = []
        current_version_fallback: list[int] = []
        current_version_ages: list[float] = []
        ...
        while len(data) < target_data_size:
            ...
            if oldest is not None or finish_version is not None:
                ...
                if query_available and (age := getattr(self._weight_version, "success_age_seconds", None)) is not None:
                    current_version_ages.append(age)
            if newest is not None:
                finish_version = max(finish_version or newest, newest)
            # Preserve the existing best-effort filter behavior, but keep the
            # authoritative observation separate so metrics never report an
            # unknown post-finish lag as zero.
            current = queried_current
            ...
            if finish_version is not None and query_available:
                post_finish_lag = queried_current - finish_version
