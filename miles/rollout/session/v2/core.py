...
class SessionCoreV2(SessionCore):
    ...
    async def chat_completions(
        ...
        async with session.lock:
            ...
            # position_for_request moved the view to the attach node, so the
            # active snapshot is the checkpoint this request builds on (a branch
            # re-requests the rows past its ancestor, like a v1 rollback).
