...
def uses_truncated_sampling(*, top_p: float, top_k: int) -> bool:
    """Whether sampling normalizes over less than the complete vocabulary."""
    return top_p < 1.0 or top_k > 0


def sampling_mask_replay_enabled(args: Namespace) -> bool:
    """Whether the rollout configuration requires exact support replay."""
    return uses_truncated_sampling(
...
def should_return_sampling_mask(
    ...
    """Validate one request against the configured exact-replay contract."""
    configured = sampling_mask_replay_enabled(args)
    params = sampling_params or {}
    request_uses_truncation = uses_truncated_sampling(
        top_p=float(params.get("top_p", getattr(args, "rollout_top_p", 1.0))),
        top_k=int(params.get("top_k", getattr(args, "rollout_top_k", -1))),
    )
    if request_uses_truncation != configured:
        raise ValueError(
            "request-level top-p/top-k cannot change whether sampling is truncated; "
            "set --rollout-top-p/--rollout-top-k to enable exact sampling-support replay for the run"
        )
    return configured
