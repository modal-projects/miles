...
def _resolve_hook(path: str):
    # Resolve once per path (import + getattr), not per /generate request.
...
async def apply_rollout_request_hook(
    ...
    *,
    ...
    result = _resolve_hook(args.custom_rollout_request_hook_path)(args, sample, request)
...
async def abort(args: Namespace, rollout_id: int) -> list[list[Sample]]:
    ...
    state.aborted = True
    discarded_opaque_requests = False
    ...
    if getattr(args, "rollout_endpoint_url", None):
        ...
        if not args.partial_rollout:
            ...
            state.pendings = set()
            discarded_opaque_requests = True
