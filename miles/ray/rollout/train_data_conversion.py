...
def put_train_data(args, data: dict[str, Any], train_parallel_config: dict | None) -> Box | list[Box] | dict:
    """Store rollout data, sharding routing replay by trainer pipeline stage."""
    store = object_store.get_instance()
    if args.delay_split_train_data_by_dp:
        return store.put(value=data, value_spec=ROLLOUT_DATA_VALUE_SPEC)
    ...
    dp_refs = [store.put(value=shard, value_spec=ROLLOUT_DATA_VALUE_SPEC) for shard in shards]
    routing_refs = _put_routing_replay_shards(
        routing_by_dp=routing_by_dp,
        routing_specs=routing_specs,
        dp_size=train_parallel_config["dp_size"],
    )
    return {"dp": dp_refs, "routing": routing_refs}
...
def _put_routing_replay_shards(*, routing_by_dp, routing_specs, dp_size: int) -> list[Box]:
    store = object_store.get_instance()
    refs_by_layout = {}
    refs_by_rank = []
