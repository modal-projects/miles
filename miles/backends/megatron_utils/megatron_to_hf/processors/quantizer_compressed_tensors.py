...
def _quantize_params_fp8_channel(args, converted_named_params, quantization_config):
    ...
    for name, param in converted_named_params:
        ...
        param = _match_native_fp8_layout(name, param, native_fp8_specs)
...
def _match_native_fp8_layout(hf_name, param, native_fp8_specs):
    if native_fp8_specs is None:
        return param
    expected = native_fp8_specs.get(hf_name)
    if expected is None:
        raise KeyError(f"{hf_name} is not stored as an FP8 weight in the native checkpoint")
    ...
    raise ValueError(
        f"Unexpected native FP8 export shape for {hf_name}: got {actual}, "
        f"expected {expected}"
    )
