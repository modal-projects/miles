...
def _quantize_params_fp8_channel(args, converted_named_params, quantization_config):
    ...
    checkpoint_specs = load_safetensors_metadata(args.hf_checkpoint)
    native_fp8_specs = {
        name: spec
        for name, spec in checkpoint_specs.items()
        if name.endswith(".weight")
        and spec.dtype in {"F8_E4M3", "F8_E4M3FN"}
        and name.removesuffix(".weight") + ".weight_scale" in checkpoint_specs
    }
    native_fp8_specs = native_fp8_specs or None
    ...
    for name, param in converted_named_params:
        ...
        results.append((name.removesuffix(".weight") + ".weight_scale", scale))
...
def _match_native_fp8_layout(hf_name, param, expected):
    ...
    if actual == expected:
        return param
    if param.dim() == 2 and len(expected) == 2 and actual == (expected[1], expected[0]):
