...
def get_moe_layer_indices(config, *, vp_stage: int | None) -> list[int]:
    """Return global MoE layer indices for one Megatron pipeline stage."""
...
def get_local_moe_layer_indices(models) -> list[int]:
    """Return the global indices of the MoE layers owned by this PP rank."""
    layer_indices = []
    for vp_stage, model in enumerate(models):
        layer_indices.extend(get_moe_layer_indices(model.module.config, vp_stage=vp_stage))
    return layer_indices
...
def register_replay_list_moe(
    ...
    **_kwargs,
    ...
    """Map a full-model or PP-local routing tensor to local MoE routers."""
    local_layer_indices = get_local_moe_layer_indices(models)
    if len(replay_list) != len(local_layer_indices):
        raise ValueError(
            f"Registered {len(replay_list)} routing replay streams for "
            f"{len(local_layer_indices)} local MoE layers"
        )
