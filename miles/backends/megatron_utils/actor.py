...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        *,
        ...
        if self.args.colocate:
            ...
            assert self.args.update_weight_transfer_mode in ("disk", "disk-delta")
            # Lazy import: the disk/delta codecs (numpy/zstandard/xxhash/safetensors)
            # stay off the default broadcast/p2p paths.
            if self.args.update_weight_transfer_mode == "disk-delta":
                from .update_weight.update_weight_from_distributed.delta import UpdateWeightFromDiskDelta

                update_weight_cls = UpdateWeightFromDiskDelta
            elif self.args.update_weight_disk_delta:
    ...
    def update_weights(self, info: "EnginesAndLock") -> None:
        ...
        engines_fresh = getattr(self.weight_updater, "is_rollout_engines_fresh", lambda: True)()

        if has_new_engines or publish_only or not engines_fresh:
