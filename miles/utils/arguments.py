...
def _resolve_rollout_functions(args) -> None:
    ...
    if args.async_keep_partial_groups_on_abort:
        assert args.fully_async, "--async-keep-partial-groups-on-abort requires --fully-async"
        assert args.async_unused_samples_handler == "drop", (
            "--async-keep-partial-groups-on-abort requires --async-unused-samples-handler drop; "
            "retrying the original prompt would duplicate the retained trajectories"
        )
        assert args.use_dynamic_global_batch_size, (
            "--async-keep-partial-groups-on-abort requires --use-dynamic-global-batch-size"
        )
        assert args.n_samples_per_prompt >= 2, (
            "--async-keep-partial-groups-on-abort requires --n-samples-per-prompt >= 2"
        )
...
def get_miles_extra_args_provider(add_custom_arguments=None):
    def add_miles_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                "--async-keep-partial-groups-on-abort",
                action="store_true",
                default=False,
                help=(
                    "In fully async mode, keep the completed trajectories from a prompt group "
                    "when at least two trajectories survive an abort. This requires dropping "
                    "unused samples and a dynamic global batch size."
                ),
            )
            parser.add_argument(
...
def _resolve_checkpoint_load(args) -> None:
    """Keep a selected training checkpoint; otherwise load the initial weights."""
    checkpoint_selected = args.load is not None and (
        args.exit_on_missing_checkpoint
        or args.ckpt_step is not None
        or os.path.isfile(os.path.join(args.load, "latest_checkpointed_iteration.txt"))
    )
