...
class TestRolloutServerPureFunctions:
    def test_opaque_endpoint_starts_no_local_engines(self):
        args = make_args(rollout_endpoint_url="https://rollout.example")

        servers = start_rollout_servers(args, pg=None)

        assert list(servers) == ["default"]
        assert servers["default"].server_groups == []
        assert servers["default"].update_weights is True
