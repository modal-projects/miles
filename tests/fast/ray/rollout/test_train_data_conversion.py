...
class TestPutTrainData:
    ...
    def test_pp_local_routing_uses_dp_and_layer_layout(self):
        ...
        compact_data = compact_routing_replay_for_transport(args, data)

        refs = put_train_data(args, compact_data, config)

        assert set(refs) == {"dp", "routing"}
        dp_shards = [ray.get(ref.inner) for ref in refs["dp"]]
        assert all("rollout_routed_experts" not in shard for shard in dp_shards)

        routing_shards = [ray.get(ref.inner) for ref in refs["routing"]]
    ...
    def test_pp_local_routing_deduplicates_identical_rank_layouts(self):
        ...
        data = compact_routing_replay_for_transport(args, self._routing_data())

        refs = put_train_data(args, data, config)

        assert refs["routing"][0] is refs["routing"][1]

    def test_without_topology_keeps_routing_in_dp_shards(self):
        args = make_args(balance_data=False, moe_router_topk=2, num_experts=128)
        data = compact_routing_replay_for_transport(args, self._routing_data())

        refs = put_train_data(args, data, {"dp_size": 2})

        assert isinstance(refs, list)
        assert all("rollout_routed_experts" in ray.get(ref.inner) for ref in refs)
