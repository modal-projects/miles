...
class TestComputeSamplesFromRecords:
    ...
    def test_compact_top_p_record_uses_configured_sampling_contract(self):
        ...
        # The configured rollout contract is authoritative even for records
        # that predate the explicit request flag.
        record.request.pop("return_sampling_mask")
    ...
    def test_compact_top_k_record_uses_configured_sampling_contract(self):
        tok = _mock_tokenizer()
        record = _make_record(
            prompt_token_ids=[1, 2, 3],
            output_token_ids=[10],
            output_log_probs=[-2.5],
            sampling_masks=[[10, 4, 7]],
            sampling_log_probs=[-0.5],
        )
        record.request.pop("return_sampling_mask")
        ...
        (sample,) = compute_samples_from_openai_records(
            ...
            [record],
            tok,
            accumulated_token_ids=[1, 2, 3, 10],
        )

        assert sample.rollout_log_probs == [-0.5]
        assert sample.rollout_sampling_mask_ids == [10, 4, 7]
        assert sample.rollout_sampling_mask_offsets == [0, 3]
        sample.validate()
