...
from miles.rollout.filter_hub.base_types import MetricGatherer, aborted_exit_status, iter_samples
...
async def test_sync_rollout_drops_aborted_group_without_a_dynamic_filter(monkeypatch):
    ...
    assert not any(sample.status == Sample.Status.ABORTED for group in output.samples for sample in iter_samples(group))
