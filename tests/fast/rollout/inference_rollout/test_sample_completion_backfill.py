...
class Harness:
    ...
    def __init__(self, monkeypatch, args: Namespace):
        ...
        monkeypatch.setattr(train, "_configure_sglang", noop_configure_sglang)
...
async def test_backfill_caps_batched_submission_to_available_slots(monkeypatch):
    harness = Harness(
        monkeypatch,
        make_args(
            rollout_batch_size=2,
    ...
    # Exactly one group slot becomes available. A batched data-source request
    # must not turn that one slot into two new in-flight groups.
    harness.finish_samples(0, GROUP_SIZE)
    await asyncio.sleep(0.01)
    assert harness.submitted_group_indices == [1, 2, 3]
