...
async def test_empty_reply_returns_aborted_list(monkeypatch, empty_reason):
    ...
    expected_status = "AllTruncated" if empty_reason == "all_truncated" else "NoModelCalls"
    assert output.samples[0].metadata["exit_status"] == expected_status
...
async def test_infra_abort_discards_the_sample(monkeypatch, use_v2):
    ...
    async def aborting_agent(**kwargs):
        raise InfraAbort("SandboxUnavailable", "quota exhausted after retries")
...
async def test_transport_collection_error_has_no_metrics_owner(monkeypatch):
    ...
    assert sample.metadata["exit_status"] == "CollectFailed"
