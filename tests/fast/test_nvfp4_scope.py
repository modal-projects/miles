...
def test_qwen_fused_experts_expand_in_a_bf16_layer(tmp_path, fake_quantizers):
    prefix = "model.language_model.layers.0.mlp.experts"
    tensors = {
        f"{prefix}.gate_up_proj": torch.arange(64, dtype=torch.float32)
        .to(torch.bfloat16)
        .reshape(2, 32, 1),
        f"{prefix}.down_proj": torch.arange(32, dtype=torch.float32)
        .to(torch.bfloat16)
        .reshape(2, 1, 16),
    ...
    assert f"{prefix}.gate_up_proj" not in actual
    assert f"{prefix}.down_proj" not in actual
    assert actual[f"{prefix}.0.gate_proj.weight"].shape == (16, 1)
    assert actual[f"{prefix}.1.down_proj.weight"].shape == (1, 16)
    assert actual["model.language_model.layers.0.linear_attn.A_log"].dtype == torch.float32
    assert prefix in ignore
