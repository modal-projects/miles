...
requires dynamic global batches. If the unused-sample handler is `retry`, Miles retries
only groups for which fewer than two trajectories survived; retained groups are not retried.
...
| `--async-keep-partial-groups-on-abort` | Keep completed trajectories from an aborted group when at least two survive. Requires `--use-dynamic-global-batch-size` |
