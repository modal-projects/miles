...
`--async-keep-partial-groups-on-abort` instead drops only the aborted trajectories
when at least two trajectories remain. Groups with zero or one survivor are still
discarded because a singleton group has no centered policy-gradient signal. This mode
requires `--async-unused-samples-handler drop` and dynamic global batches; Miles keeps
prompt groups intact when aligning the resulting variable-size batch to data parallelism.
...
| `--async-keep-partial-groups-on-abort` | Keep completed trajectories from an aborted group when at least two survive. Requires `drop` handling and `--use-dynamic-global-batch-size` |
