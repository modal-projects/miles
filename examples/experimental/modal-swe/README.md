...
Use session server v2 and set
`--session-sample-postprocessor-path modal_swe_agent_function.postprocess_samples`.
The hook delegates ordinary samples to Miles' default v2 postprocessor and
marks infrastructure-failed attempts as aborted, so they are handled by the
fully-async data buffer instead of being trained as zero-reward policy errors.
