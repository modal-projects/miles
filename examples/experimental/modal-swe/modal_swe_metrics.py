...
def add_metrics(samples: list[Sample], output: dict[str, Any]) -> None:
    if not samples:
        return
    # Masked infrastructure placeholders carry the failed attempt's metadata,
    # even though their cloned tensor fields are excluded from the loss. Keep
    # those attempts in operational timing and exit-status aggregates so
    # infrastructure regressions remain visible.
